...
public class LevelOne extends InputAdapter implements Screen {
    ...
    ProjectileEntity projectilePlayer2,projectilePlayer1;
    ...
    Texture playerTexture2,playerTextrue1, spaceTexture, puckTexture;
    ...
    public LevelOne(final MainGame game){
        ...
        //viewport = new StretchViewport(screenWidth,screenHeight, camera2);
        ...
        playerTextrue1 = new Texture("ship3.png");
        ...
        player1 = new Sprite(playerTextrue1,(int) screenHeight/5, (int)screenWidth/4);
    ...
    private void resetearPosiciones(boolean jugador) {
        ...
        //asignar la posciion de los projectiles
    ...
    public void show() {
        ...
        //comportamiento
        steeringAgent = new SteeringAgent(playerEntity1.getBody(),playerEntity1.RADIUS,200000);
    ...
    public void behaviorAI(){
        ...
        //si la bola para por algun motivo
        if(firstContact){
            if(agent.getLinearVelocity().y == 0 && agent.getLinearVelocity().x ==0){
                agent.getBody().setLinearVelocity(30,30);
            }
        }

        if(agent.getBody().getPosition().y < 0 || agent.getBody().getPosition().y > 13.5 || agent.getBody().getPosition().y < -13.5){
            arrive.setEnabled(false);
        }else {
            arrive.setEnabled(true);
        }
        //System.out.println("puck x: " + vel.x + " y: " + vel.y);
        //disparo
        if(lifePlayer1){
            if(firstContact){
                if(agent.getBody().getPosition().y < 0){
                    if(projectilePlayer1.getBody().getPosition().y < -16 || projectilePlayer1.getBody().getPosition().y > 16){
                        ...
                        projectilePlayer1.getBody().setLinearVelocity(0,-20);
                        if(game.SOUND)
        ...
        if(lifePlayer2){
            if(firstContact){
                if(projectilePlayer2.getBody().getPosition().y < -15 || projectilePlayer2.getBody().getPosition().y > 15){
                    ...
                    projectilePlayer2.getBody().setLinearVelocity(0,20);
                    if(game.SOUND)
                        game.shootSound.play(0.7f);
                }
            }
                ...
                if(fixtureA.getBody().getUserData() != null && fixtureB.getBody().getUserData() != null){
                    ...
                    || fixtureA.getBody().getUserData().equals("IA") && fixtureB.getBody().getUserData().equals("puck")){
                        ...
                        if(agent.getBody().getPosition().y > 13.4){
                            ...
                            if(agent.getBody().getPosition().x > 8.35){
    ...
    public void imprimirImagenes(){
        ...
        //batch2.draw(projectile, projectileEntity.getBody().getPosition().x-projectileEntity.RADIUS, projectileEntity.getBody().getPosition().y-projectileEntity.RADIUS, projectileEntity.RADIUS*2,projectileEntity.RADIUS*2);
    ...
    public boolean touchDragged(int screenX, int screenY, int pointer) {
        ...
        if (jointJugador2 != null && pointer == pointerPlayer2) {
            ...
            //System.out.println("jugador1 y " + playerEntity1.getBody().getPosition().x+" " + playerEntity1.getBody().getPosition().y);
