...
public class LevelFourQuarters extends InputAdapter implements Screen {
    ...
    public LevelFourQuarters(final MainGame game){
            ...
            if(firstContact){
                if(agent.getBody().getPosition().y < 0){
                    if(projectilePlayer1.getBody().getPosition().y < -16 || projectilePlayer1.getBody().getPosition().y > 16){
                        ...
                        projectilePlayer12.getBody().setLinearVelocity(0,-30);
                        if(game.SOUND)
